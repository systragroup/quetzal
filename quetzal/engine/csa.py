...
def time_zone_to_transit(links, zone_to_transit, reindex=False, step=None):
    ...
    if reindex:
        ...
        df["csa_index"] = (
            "ztt_" + df["model_index"].astype(str) + "_" + df["str"].astype(str)
        )
...
def pathfinder(
    ...
    decreasing_departure_connections = pseudo_connections[columns].to_dict(
        orient="records"
    )
    ...
    for target in tqdm(targets):
        ...
        profile, predecessor = csa_profile(
            connections, target=target, stop_set=stop_set, Ttrip=Ttrip_inf.copy()
        )
...
def drop_off_pickup_filter(
    ...
    drop_off = (
        ~connections.set_index("csa_index")["drop_off_type"].astype(bool)
    ).to_dict()
    drop_off_list = [
        [drop_off.get(c) for c in csa_path if drop_off.get(c) is not None]
        for csa_path in pt_los["csa_path"].values
    ]
    ...
    pickup_list = [
        [pickup.get(c) for c in csa_path if pickup.get(c) is not None]
        for csa_path in pt_los["csa_path"].values
    ]
...
def get_footpaths_time(
    pt_los: pd.DataFrame, pseudo_connections: pd.DataFrame, footpaths: pd.DataFrame
) -> pd.Series:
    footpaths_set = set(footpaths["model_index"])
    pseudo_footpaths = pseudo_connections[
        pseudo_connections["model_index"].isin(footpaths_set)
    ]
    pseudo_footpaths["time"] = (
        pseudo_footpaths["arrival_time"] - pseudo_footpaths["departure_time"]
    )
    footpaths_time_dict = pseudo_footpaths.set_index("csa_index")["time"].to_dict()
    return pt_los["csa_path"].apply(
        lambda ls: sum([footpaths_time_dict.get(x, 0) for x in ls])
    )
...
def merge_on_connector(
    ...
    pt_los = pt_los.rename(
        columns={"origin": "stop_access", "destination": "stop_egress"}
    )
    ...
    df["pseudo_departure_time"] = df["departure_time"] - df["time_access"] * (
        walk_penalty - 1
    )
    df["pseudo_arrival_time"] = df["arrival_time"] + (
        df["time_egress"] + df["footpath_time"]
    ) * (walk_penalty - 1)
...
def pareto_per_groups(
    all_departures: np.ndarray, all_arrivals: np.ndarray, groups: np.ndarray
) -> np.ndarray[bool]:
    """
