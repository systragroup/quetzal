...
def nearest(one, links, radius=False):
    ...
    if radius:
        indices = links.r_nbrs.radius_neighbors(
            y, radius=links.radius_search, return_distance=False
        )
    else:
        indices = links.nbrs.kneighbors(
            y, n_neighbors=links.n_neighbors_centroid, return_distance=False
        )
...
def emission_logprob(distance, SIGMA, p):
    ...
    return (
        0.5 * (distance / SIGMA) ** p
    )  # Drop constant with log. its the same for everyone.
...
def get_candidat_links(gps_track, links, method):
    if method == "knn":
        ...
        candidat_links = (
            pd.concat([candidat_links, temp_candidat])
            .sort_values("ix_one")
        ...
        candidat_links = nearest(gps_track, links, radius=True).drop(columns=["rank"])
        unfound_points = candidat_links[candidat_links["index_nn"].isnull()][
            "ix_one"
        ].values
        candidat_links = candidat_links.dropna()
        ...
        if len(unfound_points) > 0:
            ...
            temp_candidat = nearest(
                gps_track.loc[unfound_points], links, radius=False
            ).drop(columns=["rank"])
            temp_candidat["ix_one"] = temp_candidat["ix_one"].apply(
                lambda x: index_dict.get(x)
            )
            candidat_links = (
                pd.concat([candidat_links, temp_candidat])
                .sort_values("ix_one")
...
def convert_timestamp(gps_track):
    try:
        ...
        raise Exception(
            "must inclide timestamp in columns for the speed_limit=True flag"
        )

    # need pandas timestamp of ms. time in ms!!
    # convert to ms timestamp
    if type(gps_track["timestamp"][0]) == pd.Timestamp:
        gps_track["timestamp"] = gps_track["timestamp"].apply(
...
class RoadLinks:
    ...
    def __init__(
        self, links, n_neighbors_centroid=10, radius_search=250, on_centroid=False
    ):
        self.links = links
        assert self.links.crs != None, (
            "road_links crs must be set (crs in meter, NOT 3857)"
        )
        assert self.links.crs != 3857, (
            "CRS error. crs 3857 is not supported. use a local projection in meters."
        )
        assert self.links.crs != 4326, (
            "CRS error, crs 4326 is not supported, use a crs in meter (NOT 3857)"
        )
    ...
    def get_sparse_matrix(self):
        self.mat, self.node_index = sparse_matrix(
            self.links[["a", "b", "length"]].values
        )
    ...
    def get_dict(self):
        ...
        self.geom_dict_arr = {
            key: get_coordinates(item) for key, item in self.geom_dict.items()
        }
    ...
    def fit_nearest_model(self):
        ...
        interp_points_dict = {
            key: get_points_along_multiline(item, dist)
            for key, item in self.geom_dict_arr.items()
        }
        ...
        self.nbrs = NearestNeighbors(
            n_neighbors=self.n_neighbors_centroid, algorithm="ball_tree"
        ).fit(points)
        self.r_nbrs = NearestNeighbors(
            radius=self.radius_search, algorithm="ball_tree"
        ).fit(points)
    ...
    def fit_nearest_centroid(self):
        ...
        links = add_geometry_coordinates(
            self.links, columns=["x_geometry", "y_geometry"]
        )
...
def Parallel_Mapmatching(
    ...
    **kwargs,
    ...
    chunks = [
        trip_list[j : j + chunk_length] for j in range(0, len(trip_list), chunk_length)
    ]
...
def Mapmatching(
    ...
    if dijkstra_limit is None:
        dijkstra_limit = (
            add_geometry_coordinates(gps_track)[["x_geometry", "y_geometry"]]
            .std()
            .mean()
            / 2
        )
    ...
    if speed_limit:
        ...
        timestamp_dict = (
            gps_track["timestamp"].shift(-1) - gps_track["timestamp"]
        ).to_dict()
    ...
    candidat_links["road_geom_arr"] = candidat_links["index_nn"].apply(
        lambda x: links.geom_dict_arr.get(x)
    )
    candidat_links["gps_geom_arr"] = candidat_links["ix_one"].apply(
        lambda x: gps_dict_arr.get(x)
    )

    # Add gps distance to road.
    candidat_links["distance"] = point_to_line_distance(
        candidat_links["gps_geom_arr"], candidat_links["road_geom_arr"]
    )
    ...
    # Add offset
    candidat_links["road_geom"] = candidat_links["index_nn"].apply(
        lambda x: links.geom_dict.get(x)
    )
    candidat_links["gps_geom"] = candidat_links["ix_one"].apply(
        lambda x: gps_dict.get(x)
    )
    candidat_links["offset"] = project(
        candidat_links["road_geom"], candidat_links["gps_geom"], normalized=False
    )
    dict_distance = candidat_links.set_index(["ix_one", "index_nn"])[
        "distance"
    ].to_dict()

    # make tuple with road index and offset.
    candidat_links["index_nn"] = list(
        zip(candidat_links["index_nn"], candidat_links["offset"])
    )
    ...
    candidat_links.loc[len(candidat_links)] = [
        candidat_links["ix_one"].max() + 1,
        candidat_links["index_nn"].iloc[-1],
    ]
    ...
    dict_point_link = dict(
        zip(
            list(candidat_links["ix_one"].unique())[:-1],
            list(candidat_links["ix_one"].unique())[1:],
        )
    )
    ...
    candidat_links = (
        candidat_links.explode(column="road_a")
        .explode(column="road_b")
        .reset_index(drop=True)
    )
    ...
    candidat_links["node_b"] = candidat_links["road_a"].apply(
        lambda x: links.dict_node_b.get(x)
    )
    candidat_links["node_a"] = candidat_links["road_b"].apply(
        lambda x: links.dict_node_a.get(x)
    )
    ...
    temp_dist_matrix = (
        dist_matrix.stack()
        .reset_index()
        .rename(columns={"level_0": "b", "level_1": "a", 0: "dijkstra"})
    )
    ...
    unfound_origin_nodes = candidat_links[np.isnan(candidat_links["dijkstra"])][
        "node_b"
    ].unique()
    ...
    candidat_links["length"] = candidat_links["road_a"].apply(
        lambda x: links.length_dict.get(x)
    )
    ...
    candidat_links.loc[cond, "dijkstra"] = (
        candidat_links.loc[cond, "road_b_offset"]
        - candidat_links.loc[cond, "road_a_offset"]
    )
    ...
    candidat_links["distance_to_road"] = candidat_links.set_index(
        ["ix_one", "road_a"]
    ).index.map(dict_distance.get)  # .fillna(5)

    # applique la distance entre les point gps a vers b
    candidat_links["gps_distance"] = candidat_links["ix_one"].apply(
        lambda x: gps_dist_dict.get(x)
    )  # .fillna(3)

    # path prob
    candidat_links["path_prob"] = emission_logprob(
        candidat_links["distance_to_road"], SIGMA, POWER
    )
    ...
    if (turn_penalty == True) and ("bearing_dict" in links.__dict__.keys()):
        candidat_links["angle"] = candidat_links["road_a"].apply(
            lambda x: links.bearing_dict.get(x)
        ) - candidat_links["road_b"].apply(lambda x: links.bearing_dict.get(x))
        candidat_links["path_prob"] += turning_penalty_logprob(
            candidat_links["angle"], BETA
        )

    if speed_limit:
        # (dist/1000)/(time/1000/3600) # speed in kmh
        candidat_links["gps_time"] = (
            candidat_links["ix_one"].apply(lambda x: timestamp_dict.get(x, 0))
            / 1000
            / 3600
        )
        candidat_links["speed"] = (candidat_links["dijkstra"] / 1000) / (
            candidat_links["gps_time"]
        )
        ...
        candidat_links.loc[
            candidat_links["road_a"] == candidat_links["road_b"], "speed"
        ] = 0
        ...
        candidat_links["path_prob"] += candidat_links["speed"].apply(
            lambda x: np.log10(max(x - MAX_SPEED, 1))
        )
    ...
    candidat_links["a"] = list(
        zip(
            candidat_links["ix_one"],
            candidat_links["road_a"],
            candidat_links["road_a_offset"],
        )
    )
    ...
    pseudo_mat, pseudo_node_index = sparse_matrix(
        candidat_links[["a", "b", "path_prob"]].values.tolist()
    )
    ...
    pseudo_predecessors[0] = pseudo_predecessors[0].apply(
        lambda x: pseudo_index_node.get(x)
    )
    ...
    val = pd.DataFrame(temp_path, columns=["index", "road_id", "offset"]).set_index(
        "index"
    )[1:]
    ...
    dijkstra_dict = candidat_links.set_index(
        ["ix_one", "road_a", "road_b"], drop=False
    )["dijkstra"].to_dict()
    val["length"] = val.set_index([val.index, "road_id_a", "road_id_b"]).index.map(
        dijkstra_dict.get
    )
    ...
    if routing:
        ...
        df_path["from_a"] = df_path["road_id"].apply(
            lambda x: links.node_index.get(links.dict_node_a.get(x))
        )
        df_path["from_b"] = df_path["road_id"].apply(
            lambda x: links.node_index.get(links.dict_node_b.get(x))
        )
        ...
        node_mat = []
        for ori, des, from_a, to_a in df_path[
            ...
            while (
                node != -9999
            ):  # Ajoute les noeds b jusqua ce qu'on arrive au prochain point gps
                node_list.append(node)
                node = routing_predecessors.loc[ori, node]
            node_list.append(from_a)
            node_list = [
                index_node[x] for x in node_list[::-1]
            ]  # reverse and swap index
        ...
        if plot:
            ...
            plot_lineStrings(
                links.links[links.links["index"].isin(ls)],
                ax=ax,
                color="orange",
                linewidth=2,
            )
            plt.xlim(
                [
                    gps_track["geometry"].x.min() - 1000,
                    gps_track["geometry"].x.max() + 1000,
                ]
            )
            plt.ylim(
                [
                    gps_track["geometry"].y.min() - 100,
                    gps_track["geometry"].y.max() + 1000,
                ]
            )
            plt.show()
        return val, node_mat
...
def duplicate_nodes(original_links, original_nodes):
    ...
    if len(nodes_dup_list) > 0:  # skip if no duplicated nodes
        ...
        # new name!
        nodes_dup_list["new_index"] = (
            nodes_dup_list["index"].astype(str)
            + "-"
            + nodes_dup_list["trip_id"].astype(str)
        )
        ...
        new_index_dict = nodes_dup_list.set_index(["trip_id", "index"])[
            "new_index"
        ].to_dict()
        new_index_dict = nodes_dup_list.set_index(["trip_id", "index"])[
            "new_index"
        ...
        if len(nodes_dup_list[nodes_dup_list["index"].duplicated()]) > 0:
            print(
                "there is at least a node with duplicated (index, trip_id), will not be split in different nodes"
            )
            print(nodes_dup_list[nodes_dup_list["new_index"].duplicated()]["new_index"])

        # duplicate nodes and concat them to the existing nodes.
        new_nodes = nodes_dup_list[["index", "new_index"]].merge(
            original_nodes, left_on="index", right_on="index"
        )
        new_nodes = new_nodes.drop(columns=["index"]).rename(
            columns={"new_index": "index"}
        )
        ...
        original_links["new_a"] = original_links.set_index(["trip_id", "a"]).index.map(
            new_index_dict
        )
        original_links["a"] = original_links["new_a"].combine_first(original_links["a"])

        original_links["new_b"] = original_links.set_index(["trip_id", "b"]).index.map(
            new_index_dict
        )
