...
def find_road_node_options(node_path, nearest_neighbors):
    ...
    for node_index in range(len(node_path)):
        ...
        road_nodes = df.loc[df['ix_one'] == node, 'ix_many'].drop_duplicates().tolist()
        tuples = [(node, road_node) for road_node in road_nodes]
        options[node] = tuples
    return options


def build_shortcut_ghaph(node_path, road_node_options, road_graph, penalties=None):
    if True:
    ...
    node_tuples = [(node_path[i], node_path[i + 1]) for i in range(len(node_path) - 1)]
    ...
    for tuple_index in range(len(node_tuples)):
        ...
        product = list(itertools.product(road_node_options[origin], road_node_options[destination]))
    ...
    for edge_index in range(len(edges)):
        ...
        try:
            length, path = nx.bidirectional_dijkstra(road_graph, road_origin, road_destination)
        except NetworkXNoPath:
            length, path = float('inf'), ['wrong']

        length += penalty(origin) + penalty(destination)

        weighted_edge = (origin, destination, length)
    ...
    shortcut_graph.add_weighted_edges_from(weighted_edges + o_edges + d_edges)
